...
const TaskListTable = ({tableData}) => {
  const getStatusBadgeColor = (status) => {
    switch (status) {
      case 'Done': return 'bg-green-100 text-green-500 border border-green-200';
        ...
        {safeData.map((task) => (
          <tr key={task._id} className='border-t border-gray-200'>
            ...
            <td className='py-4 px-4'>
              <span className={`px-2 py-1 text-xs rounded inline_block ${getStatusBadgeColor(task.status)}`}>{task.status}</span>
            </td>
            <td className='py-4 px-4'>
              <span className={`px-2 py-1 text-xs rounded inline_block ${getPriorityBadgeColor(task.priority)}`}>{task.priority}</span>
            </td>
